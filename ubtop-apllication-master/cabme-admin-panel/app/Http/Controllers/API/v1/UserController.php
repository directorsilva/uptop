    ...
    public function register(Request $request)
    {
        ...
        if ($account_type == "customer") {
            ...
            if (!empty($chkephone) or !empty($chkemail)) {
                ...
                $insertdata = DB::insert("insert into tj_conducteur(online,prenom,nom,phone,mdp,statut,login_type,tonotify,creer,updated_at,status_car_image,statut_vehicule,email,address,amount)
                values('yes','" . $prenom . "','" . $nom . "','" . $phone . "','" . $mdp . "','no','" . $login_type . "','" . $tonotify . "','" . $date_heure . "','" . $date_heure . "','no','no','" . $email . "','','0')");
                $id = DB::getPdo()->lastInsertId();
                //$id = mysqli_insert_id($con);
                if ($id > 0) {
                    $response['success'] = 'success';
