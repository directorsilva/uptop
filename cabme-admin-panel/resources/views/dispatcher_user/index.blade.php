...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">{{trans('lang.dispatcher_user')}}</h3>
        </div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{url('/dashboard')}}">{{trans('lang.dashboard')}}</a></li>
                <li class="breadcrumb-item active">{{trans('lang.dispatcher_user')}}</li>
            </ol>
        </div>
        <div>
        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="userlist-topsearch d-flex mb-3">
                            <div class="userlist-top-left">
                                <a class="nav-link" href="{!! route('dispatcher-users.create') !!}"><i
                                            class="fa fa-plus mr-2"></i>{{trans('lang.dispatcher_user_create')}}</a>
                            </div>
                            <div id="users-table_filter" class="ml-auto">
                                <label>{{ trans('lang.search_by')}}
                                <div class="form-group mb-0">
                                    <form action="{{ route('dispatcher-users') }}" method="get">
                                        ...
                                        <select name="selected_search" id="selected_search"
                                                class="form-control input-sm">
                                            <option value="first_name">{{ trans('lang.user_name')}}</option>
                                        ...
                                        <div class="search-box position-relative">
                                            ...
                                            <a class="btn btn-warning btn-flat"
                                               href="{{url('dispatcher-users')}}">Clear</a>
                                        </div>
                                    </form>
                                </div>
                                </label>
                        ...
                        <div class="table-responsive m-t-10">
                            <table id="example24"
                                ...
                                <tr>
                                    ...
                                    @if (file_exists(public_path('assets/images/dispatcher_users'.'/'.$customer->profile_picture_path))
                                    && !empty($customer->profile_picture_path))

                                    <td><img class="rounded" style="width:50px"
                                             src="{{asset('assets/images/dispatcher_users/' . $customer->profile_picture_path)}}"
                                             alt="image"></td>
                                    @else
                                    <td><img class="rounded" style="width:50px"
                                    ...
                                    @endif
                                    <td><a href="{{route('dispatcher-users.show', ['id' => $customer->id])}}">{{
                                    ...
                                    <td><label class="switch"><input type="checkbox" checked id="{{$customer->id}}"
                                                                     name="publish"><span
                                                    class="slider round"></span></label></td>

                                    @else
                                    <td><label class="switch"><input type="checkbox" id="{{$customer->id}}"
                                                                     name="publish"><span
                                                    class="slider round"></span></label></td>
                                    @endif

                                    <td class="action-btn">
                                        <a href="{{route('dispatcher-users.show', ['id' => $customer->id])}}" class=""
                                           data-toggle="tooltip" data-original-title="Details"><i
                                                    class="fa fa-eye"></i></a>
                                        <a
                                                href="{{route('dispatcher-users.edit', ['id' => $customer->id])}}"><i
                                                    class="fa fa-edit"></i></a><a id="'+val.id+'"
                                                                                  class="do_not_delete"
                                                                                  name="user-delete"
                                                                                  href="{{route('dispatcher-users.delete', ['id' => $customer->id])}}"><i
    ...
    $("#deleteAll").click(function () {
        if ($('#example24 .is_open:checked').length) {
            if (confirm('Are You Sure want to Delete Selected Data ?')) {
                ...
                var url = "{{url('dispatcher-users/delete', 'id')}}";
