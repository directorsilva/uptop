					  ...
					  -> where('tj_conducteur.statut', '=', 'yes')
						-> where('tj_conducteur.is_verified', '=', '1')
						 ...
						 -> where('tj_conducteur.amount', '>=', $amount)
						  -> where('tj_type_vehicule.id', '=', $id_cat_taxi) -> get();
		...
		if ($sql -> count() > 0) {
			...
			foreach ($sql as $row) {
				...
				if ($row -> latitude != '' && $row -> longitude != '')
					...
					$sql_nb_avis = DB::table('tj_note') -> select(DB::raw("COUNT(id) as nb_avis"), DB::raw("SUM(niveau) as somme"))
					-> where('id_conducteur', '=', $id_conducteur) -> get();
				...
				$sql_total = DB::table('tj_requete') -> select(DB::raw("COUNT(id) as total_completed_ride"))
				->where('id_conducteur', '=', $id_conducteur) -> where('statut', '=', 'completed') -> get();
