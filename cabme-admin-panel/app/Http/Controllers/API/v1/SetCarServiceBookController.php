...
use DB;
class SetCarServiceBookController extends Controller
{
  ...
  public function updateCarServiceBook(Request $request)
  {
        ...
        } else
        {
            ...
            $filename = 'Driver_Car_Service_'.$time;
            $file->move(public_path('assets/images/driver'), $filename);


        $updatedata = DB::update('update tj_conducteur set photo_car_service_book = ?,photo_car_service_book_path = ?,modifier = ? where id = ?',[$image,$filename,$date_heure,$id_user]);

        if($updatedata > 0){


            //$updatedata = DB::update('update tj_conducteur set statut_car_service_book = ? where id = ?',['uploaded',$id_user]);
          ...
          if($licence != ''){
            ...
            $roadWorthy = $row['photo_road_worthy_path'];
            $roadWorthy_path = asset('assets/images/driver').'/'.$roadWorthy;
            $row['photo_road_worthy_path'] = $roadWorthy_path;
